...
def find_entropy_summary_files(outputs_dir):
    ...
    for project_dir in outputs_path.iterdir():
        if project_dir.is_dir():
            ...
            if candidates:
                # Prefer the canonical "{project}_2020_2025_entropy_summary.csv"
                # file. The old "longest filename = most specific" heuristic
                # backfires when a project directory also contains a stale
                # duplicate from an earlier pipeline iteration whose filename
                # happens to be longer (e.g. AdminLTE's canonical file is
                # "AdminLTE_2020_2025_entropy_summary.csv" but a typo'd stale
                # run left "AminLTE_2020_2025_health_docs_entropy_summary.csv",
                # which is longer and was being picked instead).
                canonical = project_dir / f"{project_name}_2020_2025_entropy_summary.csv"
