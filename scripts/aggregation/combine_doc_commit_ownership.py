...
def find_ownership_files(outputs_dir: Path, keyword: str):
    ...
    for project_dir in outputs_dir.iterdir():
        ...
        # Prefer the canonical "{project}_2020_2025_{keyword}.csv" file: some
        # project directories contain a second, older, non-canonical file
        # (e.g. "5yr_health_docs_ownership_summary.csv" from an earlier
        # pipeline iteration) that also matches the keyword substring. Picking
        # by unordered directory-iteration order is non-deterministic across
        # filesystems/runs; anchoring on the canonical filename avoids that.
        canonical = project_dir / f"{project_name}_2020_2025_{keyword}.csv"
